		...
		IF row.composedof IS NOT NULL THEN		
			FOREACH nodeid IN ARRAY row.composedof LOOP -- Parcourt la liste des noeuds composant chaque elt de la table way
					...
					SELECT id, lon as longitude, lat as latitude
						FROM node
						WHERE node.id = nodeid AND node.visible IS TRUE AND node.datemodif <= row.datemodif
						ORDER BY datemodif DESC
						LIMIT 1
						INTO result;
...
-- Ajout d'index
CREATE INDEX lat_max_idx  ON public.way  USING btree  (lat_max);
-- DROP INDEX public.lat_min_idx;
CREATE INDEX lat_min_idx  ON public.way  USING btree  (lat_min);
-- Index: public.lon_max_idx
-- DROP INDEX public.lon_max_idx;
CREATE INDEX lon_max_idx  ON public.way  USING btree  (lon_max);
-- Index: public.lon_min_idx
-- DROP INDEX public.lon_min_idx;
CREATE INDEX lon_min_idx  ON public.way  USING btree  (lon_min);

SELECT min(id_serial), max(id_serial) from way;
SELECT way_geom(1,6841195);
